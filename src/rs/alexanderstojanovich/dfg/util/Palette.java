 ...
 */
package rs.alexanderstojanovich.dfg.util;
...
import rs.alexanderstojanovich.dfg.gui.GUI;
...
public class Palette {
    ...
    // A - STATIC METHODS
    //--------------------------------------------------------------------------     
    // load palette with given file name
    public static void load(String fileName) {
        ...
        if (buff != null) {
            ...
            for (int i = 0; i < buff.length / 3; i++) {
                ...
                colBuff[4 * index + 3] = (byte) ((i == 0) ? 0 : 0xFF);
                index++;
            }
            loaded = true;
        }
    ...
    // generate 6-bit RGB palette (64 colors)
        ...
        for (int r = 0; r < 4; r++) {
            for (int g = 0; g < 4; g++) {
                for (int b = 0; b < 4; b++) {
                    ...
                    colBuff[4 * index + 1] = (byte) col.getGreen();
                    colBuff[4 * index + 2] = (byte) col.getBlue();
                    colBuff[4 * index + 3] = (byte) ((index == 0) ? 0 : 0xFF);
                    index++;
                }
            }
        }
    ...
    // generate 8-bit RGB palette (256 colors)
